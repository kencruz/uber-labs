const createResItemCheckOut = (dishId, dishName, dishImg, dishPrice, dishDescription, dishQuantity) => {
        ...
        .then((data) => {
          ...
          $.get('api/cart')
            .then((data) => {
              ...
              $('#item-container').html('');
              data.forEach(e => $('#item-container').append(createResItemCheckOut(e.id, e.name, e.img, e.price, e.description, e.quantity)));
            });
        })
